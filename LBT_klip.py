...
def klip_dataset(dataset, mode='ADI', outputdir=".", fileprefix="", annuli=1, subsections=1, movement=1,
                 numbasis=None, numthreads=None, minrot=0, annuli_spacing="constant", maxnumbasis=None, corr_smooth=1, algo='klip', skip_derot=False, time_collapse="mean"):
    ...
    klipparams = "mode={mode},annuli={annuli},subsect={subsections},minmove={movement}," \
                     "numbasis={numbasis}/{maxbasis},minrot={minrot}, time_collapse={weighted}, skip_derot={skip_derot}".format(mode=mode, annuli=annuli, 
    ...
    pyklip_args = {'OWA':dataset.OWA, 'mode':mode, 'annuli':annuli, 'subsections':subsections, 'movement':movement,
                    ...
                    'annuli_spacing':annuli_spacing, 'maxnumbasis':maxnumbasis, 'corr_smooth':corr_smooth, 'algo':algo, 'compute_noise_cube':weighted, 'verbose':True}
    ...
    klip_output = klip_parallelized(dataset.input, dataset.centers, dataset.PAs,
                                dataset.filenums,
    ...
    dataset.output_centers[:, 0] = klipped_center[0]
    dataset.output_centers[:, 1] = klipped_center[1]
    ...
    rot_imgs = rotate_imgs(dataset.output, flattend_parangs, flattened_centers, numthreads=numthreads, flipx=dataset.flipx, new_center=aligned_center)
    ...
    KLmode_cube = collapse_data(dataset.output, pixel_weights, axis=1, collapse_method=time_collapse)
...
def klip_parallelized(imgs, centers, parangs, filenums, IWA, OWA=None, mode='ADI', annuli=1, subsections=1, movement=1,
                      numbasis=None, numthreads=None, minrot=0, maxrot=360, aligned_center=None,
                      annuli_spacing="constant", maxnumbasis=None, corr_smooth=1, algo='klip', compute_noise_cube=False, verbose = True):
    """
    ...
    global tot_iter
    tot_iter = len(phi_bounds) * len(rad_bounds)
    ...
    klipped_values = []
    klipped_indices = []
    output_imgs = np.nan*np.ones((img_shape[1] * img_shape[2], img_shape[0]) + numbasis.shape)
    ...
    for radstart, radend in rad_bounds:
        for phistart, phiend in phi_bounds:
                klipped_value, klipped_index = _klip_section_multifile(imgs, parangs, centers, filenums, numbasis,
                                                                    maxnumbasis,
                                                                    radstart, radend, phistart, phiend, movement,
                                                                    aligned_center, minrot, maxrot,
                                                                    mode, corr_smooth, algo, verbose)
                klipped_values.append(klipped_value)
                klipped_indices.append(klipped_index)
            
    for i in range(len(klipped_indices)):
        output_imgs[klipped_indices[i]] = np.swapaxes(klipped_values[i], 0, 1)
    sub_imgs = np.swapaxes(output_imgs, 0, 1)
    sub_imgs.shape = img_shape + numbasis.shape
...
def _klip_section_multifile(aligned_imgs, parangs, centers, filenums, numbasis, maxnumbasis, radstart, radend, phistart,
                            phiend, minmove, ref_center, minrot, maxrot, mode, corr_smooth=1, algo='klip', verbose=True):
    """
    ...
    """
    original_shape = aligned_imgs.shape
    aligned_shape = np.copy(original_shape)
    ...
    aligned_imgs.shape = (aligned_shape[0], aligned_shape[1] * aligned_shape[2])
    ...
    with Pool(12) as pool:
        ...
        results,_ = zip(*tqdm(pool.imap(perFileKLIP((aligned_imgs, parangs, filenums, section_ind, ref_psfs_mean_sub, covar_psfs, corr_psfs, (radstart + radend) / 2.0, numbasis, maxnumbasis, minmove, minrot, maxrot, mode, algo, verbose)), iterator), total=len(filenums)))
    ...
    n_workers = min(12, len(filenums))
    
    results = []
    with ThreadPoolExecutor(max_workers=n_workers) as exe:
        # Run evaluate params 
        futures = [exe.submit(_klip_section_multifile_perfile, aligned_imgs, parangs, filenums, file_index, section_ind, ref_psfs_mean_sub, covar_psfs, corr_psfs,
    ...
    # Non-parallelized
    for file_index, parang, filenum in tqdm(zip(range(len(filenums)), parangs, filenums)):
        try:
            output_klipped[file_index]  =  _klip_section_multifile_perfile(aligned_imgs, parangs, filenums, file_index, section_ind, ref_psfs_mean_sub, covar_psfs, corr_psfs,
                                            parang, filenum, (radstart + radend) / 2.0, numbasis,
                                            maxnumbasis, minmove, minrot, maxrot, mode,
                                            algo, verbose)
        except (ValueError, RuntimeError, TypeError) as err:
...
class perFileKLIP(object):
    ...
    def __call__(self, params):
        ...
        aligned_imgs, pa_imgs, filenums_imgs, section_ind, ref_psfs, covar,  corr, avg_rad,\
...
def _klip_section_multifile_perfile(aligned_imgs, pa_imgs, filenums_imgs, img_num, section_ind, ref_psfs, covar,  corr, parang, filenum, avg_rad,
...
class LBT(object):
    ...
    def __init__(self, filepaths=None, PAs=None, framew=512, frameh=512, numthreads=-1, origin=None):
        """
        Initialization code for LBT Data
